...
\item Assume target block time to be \texttt{2} seconds. \todo{how was this determined?}
\item Assume each block to be at most full by 25\%.
...
to execute is assumed to be 1 second per block. \todo{how was this determined?}
...
how much slower web-assembly can be, which current is \texttt{5}. \todo{how was this determined?}
...
benchmarks might not showcase that.} \todo{How is that bump calculated?}
\SubItem{Lets be a little more pessimistic and round everything up.}
\item Choose a very small number to be the fee for the absolute cheapest transaction.
Based on this, the weights should be scaled one last time to obtain this final fee.
\todo{"currently not implemented in code", still valid?}
\end{itemize}
...
Relevant fixed values for Weight assignment:

\begin{center}
  \begin{tabulary}{1.0\textwidth}{LLL}
    \textbf{Variable} & \textbf{Value} & \textbf{Description} \\
    target block time & 2 seconds & The target block time. \\
    \textit{wasm-factor} & 5 seconds & Estimate of how much slower Wasm can be. \\
...
\todo{Is \texttt{target\_block\_fullness} the minimum requirement of transactions per block?}
