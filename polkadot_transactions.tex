...
\subsection{Assigning Weight values}
The simple methodology for obtaining weight values is as follows:

\begin{itemize}
\item Benchmark as many of the function in the Polkadot runtime as possible.
\SubItem{Benchmarks are using the native execution environment.}
\item Assume target block time to be \verb|2| seconds.
\item Assume each block to be at most full by 25\% to be conservative.
\verb|target_block_fullness = 0.25|.
\item Due to errors in benchmarks and the fact that benchmarks are not measuring
the worstcase scenario, take 1 second apart for any unexpected behavior or delays.
Hence, the available time for functions to execute is assumed to be 1 second per block.
\item Since weights should be a representation of the time it takes to execute
a function, apply the target block fullness to the time, yielding 250 million nano
seconds as the available block execution time.
\item Define the total weight units available per block to be \verb|1'000'000'000|.
Applying the same \verg|target_block_fullness|.
\SubItem{\verb|total_block_weight = 1'000'000'000|}
\SubItem{\verb|available_block_weight = 250'000'000|}
\item Multiply all execution times values by a wasm-factor, an estimate of how much
slower web-assembly can be. Currently, this value is 5.
\item Divide the benchmarked execution time of each function by
\verb|available_block_execution_time| to obtain what ration of the block time it
consumes.
\item Multiply each obtained wasm-execution-time ration per function, by the
obtained \verb|available_block_weight|. This yields the actual weight of each
dispatch.
\item Round up these numbers by multiplying them to a value between 0 and 10
...
benchmarks might not showcase that.}
\SubItem{Be a bit more pessimistic and round everything up.}
