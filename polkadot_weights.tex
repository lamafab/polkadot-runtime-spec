...
implications on those design choices.
\newline
...
The Runtime reuses components/primitives to interact with the state. The behavior and
...
Single value inserts and fetches do not excessively increase performance of varying data or information.
However, excessive IO must be accounted for when the Runtime function conducts those operations in loops
or iterations over lists of items.
\newline
...
\begin{itemize}
  ...
  \item \textbf{Size of data} - Depending on the function, the data that gets inserted and fetched from
  storage can vary in sizes. The function should be examined to determine which conditions can result in
  operations on big data sets. One should also carefully consider what kind of data gets passed on to the
  Runtime functions as a input Parameter.
\end{itemize}
