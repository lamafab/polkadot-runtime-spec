...
that could have heavy implications on those design choices.
\newline

Section \ref{sect:practical-examples} walks through two practical examples of such analysis.
...
storage is required before a specific benchmark can be reliably measured. This is described in more precise
...
the execution cost of those primitives can be studied and a Weight can be applied for each use
...
\begin{itemize}
  ...
  \item \textbf{Map}: Operations on mulitple values with their corresponding, unique keys.
  \newline\newline
  The final key-value pair is stored under the key:\newline
  ...
  \newline\newline
  The final key-value pair is stored under the key:\newline
  \verb|hash(module_prefix) + hash(storage_prefix) + hash(encode(key1)) + hash(encode(key2))|.
\end{itemize}

Which type to use depends on the functionality of the Runtime module (or its sub-processes, rather).
In some cases only a single value is requires. In others, multiple values need to be operated on.
The types \textbf{Map} and \textbf{Double map} support multiple hashing algorithms, each providing different
properties in terms of safety (collision resistance) and performance:

\begin{itemize}
  \item Blake2 128-bit concat (hash + key concatenated)
  \item xxHash 64-bit concat (hash + key concatenated)
  \item Un-hashed, raw key
\end{itemize}
...
types are used in the Runtime function, on whether single values are returned or entire lists
of datasets. Single value operations are generally quite cheap and its execution time does
...
Maps, in contrast, can have additional overhead when inserting or retrieving
lists of datasets. Additionally, the Runtime function has to process each item inside
that list.
\newline
...
\begin{itemize}
  \item \textbf{Fixed iterations and list sizes} -
  Fixed iterations and list sizes can increase the overall cost of the Runtime functions,
  ...
  \item \textbf{Adjustable iterations and list sizes} -
  If the amount of iterations or list sizes depend on the input parameters of the caller or
  ...
  for benchmarking, the benchmarks should range from the minimum value to the maximum value.
  \newline\newline
  Note: What the maximum number should be really depends on the functionality that the Runtime function
  is trying to provide. If the choice for that number is not obvious, then it's advised to run
  benchmarks on a big range of values and pick a conservative number below the \verb|targeted time per block|
  limit as described in section \ref{sec:limitations}.
  \item \textbf{Input parameters} -
  Input parameters that users pass on to the Runtime function can result in dangerous consequences.
  Depending on the data type, it can be appropriate the add additional Weights based on certain properties,
  such as data size. The Runtime defines the maximum size for such cases. If it doesn't, it unconditionally
  has to and the Runtime module must be adjusted.
  \newline\newline
  When selecting parameters for benchmarking, the benchmarks should range from small values to the maximum
  value.
\end{itemize}

\subsection{Practical examples}\label{sect:practical-examples}
