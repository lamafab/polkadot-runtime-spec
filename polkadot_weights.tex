...
In Polkadot, Controller accounts can save information about them on chain, known as the "Identity Info".
This includes information such as display name, legal name, email address, and so on. Polkadot selects
a set of registrar which can judge identities and therefor incentivize a reputation model. The judgement itself
is done offchain. The registrars rating, however, is saved on chain, directly in the corresponding accounts
...
from a specific registrar. This function reveals multiple design choices that can impact performance.
\newline
