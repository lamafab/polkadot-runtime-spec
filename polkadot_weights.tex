...
from a specific registrar. This function has reveals multiple design choices that can impact performance.

The function:

\begin{itemize}
  \item fetches a list of current registrars from storage.
  \item searches that list for the specified registrar index.
  \item searches storage for the identity info of the transaction sender.
  \item searches the identity info for the specified registrar. If no entry is found, the function will
  insert the registrar into the identity info (assuming the registrar is not \"stickied\"). If the registrar
  index already exists in the identity info, it will be inserted.
  \item reserves a balance for the fee from the registrar.
  \item updates the identity info into storage.
\end{itemize}}
