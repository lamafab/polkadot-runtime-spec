...
is required before a specific benchmark gets executed. This is described in more precise detail in section
\ref{sect:preliminary-work}.
...
IO operation for each of them. The function does a lot of math, but those parts are very fast and do not require
a closer inspection in this section of the paper.
\newline

First, this function makes some basic checks to verify if the specified era not higher then the current era
...
Next, the function fetches the era reward points from storage.
...
Then, the nominators receive a payout. The functions loops through the exposed nominator list, conducting
a insertion into storage for the creation of a deposit event for each of the nominators.
