...
\begin{itemize}
  \item \textsc{Add-To($collection$, $time$)} - Adds a returned time measurement ($time$)
  to $collection$.
  \item \textsc{Create-User($name$, $index$)} - Creates a Blake2 hash of the concatenated input
  of $name$ and $index$ representing the address of a account.
  \item \textsc{Set-Balance($address$, $balance$)} - Sets a initial $balance$ for the specified
  account ($address$)
\end{itemize}
