...
Single value inserts and lookups do not extraordinary increase performance of varying data or information.
...
\begin{itemize}
  \item \textbf{Fixed iterations} -
  Fixed iterations can increase the overall cost of the Runtime functions, but the iteration
  amount does not
  change since it's not dependent on any input parameters or environment. A base Weight is appropriate
  in this case.
  \item \textbf{Adjustable iterations} -
  If the amount of iterations depends on the input parameters of the caller or specific entries in storage,
  then a certain Weight should be applied for each (additional) iteration.
  The Runtime defines the maximum value for such cases. If it doesn't, it unconditionally should
  and must be adjusted.
  \newline\newline
  When selecting parameters
  for benchmarking, the benchmarks should range from the minimum value to the maximum value.
  \item \textbf{Size of data} -
  Usually, a caller does not have
\end{itemize}

\subsubsection{"Map" type}

\subsubsection{"Double map" type}
