...
\begin{itemize}
  ...
  \item \textbf{Map}: Operations on mulitple values, datasets, with their corresponding, unique keys.
  \newline\newline
  The final key-value pair is stored under the key:\newline
...
types are used in the Runtime function, on whether single values are operated on or entire lists
...
\begin{itemize}
  ...
  such as data size. The Runtime must define limits of those properties. If it doesn't, it unconditionally
...
In order to select useful parameters, the Runtime functions have to be analysed in order
to understand which behaviors or conditions can result in expensive execution times,
as described in section \ref{sect:primitive-types}.
...
This is described in more detail in section \ref{sect:primitive-types}.
\newline
