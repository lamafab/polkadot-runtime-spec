...
funds from the staking management system to be ready for transfer. The benchmark I/O heavy since it is
configured to first create the accounts, bond a certain balance of funds, unbond those funds and then
...
\begin{algorithm}[H]\label{sec:algo-benchmark-transfer}
  ...
  \Fn{\textsc{Main}}{
    \For{$index\gets1$ \KwTo $1'000$ \KwBy $1$}{
      $stash \leftarrow$ \textsc{Create-User(\textit{"stash"}, $index$)}\;
      $controller \leftarrow$ \textsc{Create-User(\textit{"controller"}, $index$)}\;
      \textsc{Set-Balance($stash$, 100)}\;
      \textsc{Set-Balance($controller$, 100)}\;
      \textsc{Bond($controller$, 10)}\;
      \textsc{UnBond($controller$, 10)}\;
      $time \leftarrow$\textsc{Timer(Withdraw-Unbonded($sender$, $recipient$, $balance$))}\;
      \textsc{Add-To($collection$, $time$)}\;
    }
  }
  \BlankLine
...
\begin{itemize}
  ...
  \item \textsc{Set-Balance($address$, $balance$)}
      \SubItem{Sets a initial $balance$ for the specified account ($address$) in the storage state.}
  \item \textsc{Bond($address$, $amount$)}
  \item \textsc{UnBond($address$, $amount$)}
  \item \textsc{Withdraw-Unbonded($sender$, $recipient$, $balance$)}
  \item \textsc{Add-To($collection$, $time$)}
      \SubItem{Adds a returned time measurement ($time$) to $collection$.}
  \item \textsc{Timer($function$)}
      \SubItem{Measures the time from the start of the specified $function$ to its completion.}
