...
\begin{itemize}
  \item \textbf{Value}: Single value where no other keys are used.
  \newline\newline
  The final value is stored under the key:\newline
  \verb|hash(module_prefix) + hash(storage_prefix)|.
  \item \textbf{Map}: Multiple values with their corresponding, unique keys.
  \newline\newline
  The final value is stored under the key:\newline
  \verb|hash(module_prefix) + hash(storage_prefix) + hash(encode(key))|.
  \item \textbf{Double map}: Just like \textbf{Map}, but uses two keys instead of one to save and retrieve
  values (parent and child key).
  \newline\newline
  The final value is stored under the key:\newline
...
\subsection{Considerations for "Value"}


\subsection{Considerations for "Map"}
