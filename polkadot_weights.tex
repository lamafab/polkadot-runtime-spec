...
In order to produce conditions where resource heavy computation or excessive IO can be observed,
the benchmarks might require some preliminary work on the environment. As a practical example,
this section describes the specifically designed benchmark of the \verb|transfer| function available
in the Polkadot Runtime. The benchmark is designed to measure the functions worst possible condition:
...
The benchmarking implementation for the Polkadot Runtime function $transfer$ is defined as follows:
\newline
